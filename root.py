...
class Main():  
    ...
    def make(self):
        ...
        for class_text in self.today_list:
            ...
            class_lab.place(x=position,y=5)
    ...
    def after(self):
        ...
        #配置动画数据
        after_txt=Label(self.aft_cl,text='下课时间到',font=('幼圆',40),fg='yellow',bg='black')
    ...
    def select(self,event):
        ...
        if num>2 and num-1!=self.select_class:#如果是点击了新的课程那么就执行更新动画
            ...
            if self.select_class==None:#如果第一次就跳过还原动画
                ...
                #创建选择label
                self.select_label=Label(self.window,text='语 数 英 物 体 自 生 地 阅',font=('幼圆',40),fg='white',bg='black')
    ...
    def change(self,event):
        num=0
        for x in range(9):#range里的数字要看下面的“text”里有多少元素，不准确会报错
            if event.x_root-self.window.winfo_x()>=self.select_label.winfo_x()+(self.select_label.winfo_reqwidth()/9*x):
                num+=1
        self.classes[self.select_class]['bg']='black'
        self.classes[self.select_class]['text']='语数英物体自生地阅'[num-1]
        self.local=self.local[:self.select_class]+'语数英物体自生地阅'[num-1]+self.local[self.select_class+1:]
